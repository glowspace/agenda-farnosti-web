		...
		app.SmartActions = function(){
			...
			var smartActions = {
			    ...
			    userLogout: function($this){
					...
					// ask verification
					$.SmartMessageBox({
						title : "<i class='fa fa-sign-out txt-color-orangeDark'></i> Odhlásit uživatele <span class='txt-color-orangeDark'><strong>" + $('#show-shortcut').text() + "</strong></span> ?",
						content : $this.data('logout-msg') || "Zabezpečení vylepšíte tím, že po odhlášení zavřete otevřenou záložku prohlížeče.",
						buttons : '[Ne][Ano]'
			
					}, function(ButtonPressed) {
						if (ButtonPressed == "Ano") {
				...
				// RESET WIDGETS  content: $this.data('reset-msg') ||
			    resetWidgets: function($this){
					
					$.SmartMessageBox({
						title : "<i class='fa fa-refresh' style='color:green'></i> Resetování vlastního nastavení do továrního",
						content : "Přejete si RESETOVAT všechny své uložené widgety a obnovit si tak původní nastavení?",
						buttons : '[Ne][Ano]'
					}, function(ButtonPressed) {
						if (ButtonPressed == "Ano" && localStorage) {
							localStorage.clear();
							location.reload();
						}
	...
	function setup_widgets_desktop() {
		...
		if ($.fn.jarvisWidgets && enableJarvisWidgets) {
			...
			$('#widget-grid').jarvisWidgets({
				...
				settingsKeyLabel : 'Obnovit nastavení?',
				deletePositionKey : '#deletepositionkey-options',
				positionKeyLabel : 'Obnovit výchozí pozice zobrazení?',
				...
				// delete btn
				deleteButton : true,
				deleteMsg:'Upozornění: Tuto akci nelze vrátit zpět!',
				...
				timestampFormat : 'Naposledy obnoveno: %d%.%m%.%y% %h%:%i%:%s%',
				refreshButton : true,
				refreshButtonClass : 'fa fa-refresh',
				labelError : 'Omlouváme se, objevila se chyba:',
				labelUpdated : 'Naposledy obnoveno:',
				labelRefresh : 'Obnovit',
				labelDelete : 'Smazat widget:',
				afterLoad : function() {
				},
 ...
 */ 
	function loadURL(url, container) {
		...
		$.ajax({
			type : "GET",
            async : true, // POZOR kvuli jquery jsem nastavil. Pokud chyba, tak tento radek SMAZAT
			url : url,
			dataType : 'html',
			...
			beforeSend : function() {
				...
				// place cog puvodne <i class="fa fa-cog fa-spin"></i>
				container.html('<h1 class="ajax-loading-animation"><img src="img/loading.gif" width="52" /> Načítání...</h1>');
			...
			error : function(xhr, status, thrownError, error) {
				var chybka;
                if(xhr.status==0){
				    chybka = 'Internetové připojení není k dispozici.<br />Obnovte spojení a zkuste načíst stránku znovu <br /><br /> <button class="btn btn-warning" onClick="var loc=\''+url+'\';location.hash=\' \';location.hash=loc;">Načíst stránku znovu</button>';
				} else {
				    chybka = xhr.status;
				}
                container.html('<h4 class="ajax-loading-error"><i class="fa fa-warning txt-color-orangeDark"></i> Chyba v adrese: <span class="txt-color-red">' + url + '</span><br/><br/>Detail chyby: ' + chybka + ' <span style="text-transform: capitalize;">'  + thrownError + '</span></h4>');
			},
			async : true 
		});
 ...
 */ 
	function drawBreadCrumb(opt_breadCrumbs) {
		...
		bread_crumb.empty(), 
		bread_crumb.append($("<!--<li>Úvod</li>-->")), a.each(function() {
